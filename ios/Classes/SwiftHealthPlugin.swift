...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        /// Handle getData
        else if call.method.elementsEqual("getIntervalDataCalories") {
            getIntervalDataCalories(call: call, result: result)
        }
        /// Handle getData
        else if call.method.elementsEqual("getIntervalDataDistance") {
            getIntervalDataDistance(call: call, result: result)
        }
        ...
        /// Handle getTotalDistanceInInterval
        else if call.method.elementsEqual("getTotalMinutesInInterval") {
            getTotalMinutesInInterval(call: call, result: result)
        }
...
//         }
        ...
        let query = HKStatisticsCollectionQuery(quantityType: quantityType, quantitySamplePredicate: predicate, options: [.cumulativeSum], anchorDate: dateFrom, intervalComponents: interval)
                ...
                do {
                    if let quantity = statisticData.sumQuantity(),
                       ...
                       let unit = self.unitDict[dataUnitKey] {
                        ...
                        print("=== Health Result \(startDate): \(dict)")
                    }
                } catch {
    ...
    func getIntervalDataCalories(call: FlutterMethodCall, result: @escaping FlutterResult) {
        let arguments = call.arguments as? NSDictionary
        let dataTypeKey = (arguments?["dataTypeKey"] as? String) ?? "DEFAULT"
        let dataUnitKey = (arguments?["dataUnitKey"] as? String)
        let startDate = (arguments?["startTime"] as? NSNumber) ?? 0
...
//         }
    ...
    func getIntervalDataDistance(call: FlutterMethodCall, result: @escaping FlutterResult) {
        let arguments = call.arguments as? NSDictionary
        let dataTypeKey = (arguments?["dataTypeKey"] as? String) ?? "DEFAULT"
        let dataUnitKey = (arguments?["dataUnitKey"] as? String)
        let startDate = (arguments?["startTime"] as? NSNumber) ?? 0
...
//         }
    ...
    func getTotalMinutesInInterval(call: FlutterMethodCall, result: @escaping FlutterResult) {
        getTotalInInterval(call: call, result: result, identifier: HKQuantityTypeIdentifier.appleExerciseTime, unit: HKUnit.minute(), asInt: false)
    }
    
    func calculateInterval(startDateMillis: Double, endDateMillis: Double) -> DateComponents {
    ...
    private func getTotalInInterval(call: FlutterMethodCall, result: @escaping FlutterResult, identifier: HKQuantityTypeIdentifier, unit: HKUnit, asInt: Bool) {
        ...
        let query = HKStatisticsQuery(
            ...
            // Debug the result
            print("Query result total: \(total)")
