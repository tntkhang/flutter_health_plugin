...
class HealthFactory {
  ...
  /// Fetch a list of health data points based on [types].
  Future<List<HealthDataPoint>> getHealthDataFromTypesCalories(
      DateTime startTime, DateTime endTime, List<HealthDataType> types) async {
    List<HealthDataPoint> dataPoints = [];

    for (var type in types) {
      final result = await _prepareQueryCalories(startTime, endTime, type);
      dataPoints.addAll(result);
    }
  ...
  /// Fetch a list of health data points based on [types].
  Future<List<HealthDataPoint>> getHealthDataFromTypesDistance(
      DateTime startTime, DateTime endTime, List<HealthDataType> types) async {
    List<HealthDataPoint> dataPoints = [];

    for (var type in types) {
      final result = await _prepareQueryDistance(startTime, endTime, type);
      dataPoints.addAll(result);
    }
  ...
  /// Prepares a query, i.e. checks if the types are available, etc.
  Future<List<HealthDataPoint>> _prepareQueryCalories(
      DateTime startTime, DateTime endTime, HealthDataType dataType) async {
    // Ask for device ID only once
    _deviceId ??= _platformType == PlatformType.ANDROID
        ? (await _deviceInfo.androidInfo).id
    ...
    return await _dataQueryCalories(startTime, endTime, dataType);
  }
  /// Prepares a query, i.e. checks if the types are available, etc.
  Future<List<HealthDataPoint>> _prepareQueryDistance(
      DateTime startTime, DateTime endTime, HealthDataType dataType) async {
    // Ask for device ID only once
    _deviceId ??= _platformType == PlatformType.ANDROID
        ? (await _deviceInfo.androidInfo).id
    ...
    return await _dataQueryDistance(startTime, endTime, dataType);
  }
  ...
  /// Fetches data points from Android/iOS native code.
  Future<List<HealthDataPoint>> _dataQueryCalories(
      DateTime startTime, DateTime endTime, HealthDataType dataType) async {
    final args = <String, dynamic>{
      'dataTypeKey': dataType.name,
      'dataUnitKey': _dataTypeToUnit[dataType]!.name,
    ...
    final fetchedDataPoints = await _channel.invokeMethod('getIntervalDataCalories', args);
  ...
  /// Fetches data points from Android/iOS native code.
  Future<List<HealthDataPoint>> _dataQueryDistance(
      DateTime startTime, DateTime endTime, HealthDataType dataType) async {
    final args = <String, dynamic>{
      'dataTypeKey': dataType.name,
      'dataUnitKey': _dataTypeToUnit[dataType]!.name,
    ...
    final fetchedDataPoints = await _channel.invokeMethod('getIntervalDataDistance', args);
  ...
  Future<double?> getTotalMinutesInInterval(
    DateTime startTime,
    DateTime endTime,
  ) async {
    final args = <String, dynamic>{
